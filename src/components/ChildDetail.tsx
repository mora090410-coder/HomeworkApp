...
const ChildDetail: React.FC<ChildDetailProps> = ({
  ...
  const handleApproveAndPayNow = async (task: Task) => {
    try {
      ...
      await updateTaskStatus(child.householdId!, task.id, 'PENDING_PAYMENT', child.id);
                ...
                return (
                  <div key={task.id} className="bg-cream border border-gold/10 rounded-2xl px-5 py-4 flex items-center justify-between mx-4 mb-3 shadow-sm active:scale-[0.98] transition-transform">
                    <div className="flex items-center gap-3">
                      <span className="text-2xl">{getTaskIconForTask(task)}</span>
                        ...
                        {(isAssigned || task.status === 'REJECTED') && (
                          <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-3 mt-2">
                              <Pencil className="w-4 h-4 text-charcoal/30 cursor-pointer hover:text-charcoal/60" onClick={() => onEditTask(task)} />
                              <Trash2 className="w-4 h-4 text-charcoal/30 cursor-pointer hover:text-crimson/60" onClick={() => setConfirmDeleteId(task.id)} />
                            </div>
                          </div>
                        )}
