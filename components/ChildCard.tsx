...
const ChildCard: React.FC<ChildCardProps> = ({ 
  ...
  return (
    <div className="flex flex-col gap-3 font-sans w-full">
      ...
      <div className="group relative bg-white/[0.03] border border-white/[0.08] rounded-[24px] p-8 transition-colors duration-300 hover:border-[#FFCC00]/30 overflow-hidden">
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[#990000] to-[#FFCC00] opacity-60" />
        ...
        <div className="flex justify-between items-start mb-8">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 bg-[#1a1a1a] border border-white/10 px-3 py-1.5 rounded-full">
            ...
            <button 
              ...
              className="w-9 h-9 flex items-center justify-center rounded-full bg-transparent text-[#666] hover:bg-white/5 hover:text-white transition-all duration-300 hover:rotate-90 cursor-pointer"
            >
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="flex flex-col gap-1">
             <span className="text-[0.6875rem] font-bold text-[#666] uppercase tracking-wider">Earned</span>
             <span className="text-[1.5rem] font-[590] text-[#4ade80] tracking-tight">{formatCurrency(earnedAmount)}</span>
          </div>
          <div className="flex flex-col gap-1 border-l border-white/[0.06] pl-4">
             <span className="text-[0.6875rem] font-bold text-[#666] uppercase tracking-wider">Pending</span>
             <span className="text-[1.5rem] font-[590] text-[#FFCC00] tracking-tight">{formatCurrency(pendingAmount)}</span>
          </div>
          <div className="flex flex-col gap-1 border-l border-white/[0.06] pl-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          >
            ...
            <Edit2 className="w-4 h-4 text-white group-hover/btn:text-[#FFCC00] transition-colors" />
            <span className="text-sm font-medium text-white group-hover/btn:text-[#FFCC00] transition-colors">Update Grades</span>
          </button>
        ...
        <div className="mt-6 pt-4 border-t border-white/[0.06] flex justify-center">
          <button 
            ...
            className={`flex items-center gap-2 text-[0.8125rem] font-bold transition-colors group/toggle cursor-pointer ${awaitingApproval.length > 0 ? 'text-orange-400 hover:text-orange-300' : 'text-[#666] hover:text-[#FFCC00]'}`}
          >
          ...
          {awaitingApproval.length > 0 && (
            <>
              ...
              {awaitingApproval.map(task => (
                <div key={task.id} className="bg-[#fb923c]/5 border-2 border-[#fb923c]/50 rounded-2xl p-5 mb-2 relative overflow-visible">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="relative">
                      <button onClick={(e) => toggleMenu(e, task.id)} className="w-7 h-7 flex items-center justify-center rounded-md text-[#888] hover:bg-white/10 hover:text-white transition-colors cursor-pointer"><MoreHorizontal className="w-4 h-4" /></button>
                      {activeMenuId === task.id && (
                        <div className="absolute right-0 top-full mt-2 w-48 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl z-50 p-1 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-4">
                    ...
                    <span className="px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-400 text-[0.625rem] font-black uppercase tracking-widest animate-pulse-orange">Awaiting Approval</span>
                    <span className="text-xs font-bold text-[#FFCC00]">Will earn: {formatCurrency(calculateTaskValue(task.baselineMinutes, hourlyRate))}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => onApproveTask(child.id, task)} className="py-2.5 bg-gradient-to-r from-[#990000] to-[#FFCC00] text-white text-[0.875rem] font-bold rounded-xl flex items-center justify-center gap-2 hover:-translate-y-0.5 transition-all shadow-lg shadow-cardinal/20 cursor-pointer"><Check className="w-4 h-4" /> Approve</button>
              ...
              {inProgress.map(task => (
                <div key={task.id} className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5 mb-2 hover:bg-white/[0.04] transition-colors group/task relative overflow-visible">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="relative">
                      <button onClick={(e) => toggleMenu(e, task.id)} className="w-7 h-7 flex items-center justify-center rounded-md text-[#666] hover:bg-white/10 hover:text-white transition-colors cursor-pointer"><MoreHorizontal className="w-4 h-4" /></button>
                      {activeMenuId === task.id && (
                        <div className="absolute right-0 top-full mt-2 w-48 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl z-50 p-1 animate-in fade-in zoom-in-95 duration-200">
                          <div className="relative group/reassign">
                            ...
                            {activeSubmenuId === 're' && (
                              <div className="absolute left-full top-0 w-44 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl ml-1 overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 ml-[2.75rem]">
                    <div className="flex items-center gap-1.5 text-[#888] text-xs font-medium"><Clock className="w-3.5 h-3.5" /> {task.baselineMinutes} min baseline</div>
                    <span className="px-2 py-0.5 rounded-full bg-[#FFCC00]/15 text-[#FFCC00] text-[0.625rem] font-black uppercase tracking-widest">Pending</span>
                  </div>
                </div>
              ))}
            </>
