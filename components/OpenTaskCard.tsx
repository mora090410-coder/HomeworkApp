...
const OpenTaskCard: React.FC<OpenTaskCardProps> = ({ 
  ...
  return (
    <div className="bg-white/[0.02] border border-white/[0.06] rounded-[16px] p-5 hover:bg-white/[0.04] transition-colors group relative overflow-visible">
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="relative" ref={menuRef}>
          <button 
            ...
            className="w-7 h-7 flex items-center justify-center rounded-md bg-white/[0.04] hover:bg-white/[0.08] text-[#888] hover:text-white transition-colors cursor-pointer"
          >
            <MoreVertical className="w-4 h-4" />
          </button>

          {isMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-[0_12px_32px_rgba(0,0,0,0.5)] z-50 animate-in fade-in zoom-in-95 duration-200">
              <div className="p-1">
                <div className="relative">
                  ...
                  {showAssignSubmenu && (
                    <div 
                      className="absolute left-full top-0 w-44 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-[60] ml-1 animate-in slide-in-from-left-2 duration-200"
                ...
                <button 
                  ...
                  className="w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium text-[#ff4444] hover:bg-red-500/10 flex items-center gap-2 cursor-pointer"
                >
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="px-2.5 py-1 rounded-full bg-blue-400/15 text-blue-300 text-[0.75rem] font-[510] uppercase tracking-wider">
          Available
        </span>
      </div>
    </div>
