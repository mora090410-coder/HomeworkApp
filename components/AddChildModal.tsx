...
const AddChildModal: React.FC<AddChildModalProps> = ({ isOpen, onClose, onAdd }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 font-sans">
      ...
      <div className="relative w-full max-w-[560px] bg-white rounded-none shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-300 border border-neutral-lightGray">

        <div className="relative z-10 px-8 pt-8 pb-6 flex items-center gap-5 border-b border-neutral-lightGray">
          <div className="flex flex-1 gap-2">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-1.5 flex-1 rounded-full bg-neutral-lightGray overflow-hidden">
          ...
          <button onClick={onClose} className="p-2 rounded-full hover:bg-neutral-mutedBg text-neutral-darkGray transition-colors cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="relative z-10 flex-1 overflow-y-auto px-8 md:px-12 py-8 scrollbar-hide">
          {step === 1 && (
            <div className="animate-in slide-in-from-right-8 fade-in duration-300">
              <div className="text-center mb-10">
                ...
                <p className="text-neutral-darkGray">Let's start with the basics</p>
              </div>
              <div className="flex flex-col items-center mb-8">
                <div className={`w-[80px] h-[80px] rounded-full flex items-center justify-center text-3xl font-bold font-heading transition-all duration-300 ${name.trim() ? 'bg-primary-cardinal text-white shadow-md' : 'bg-neutral-mutedBg text-neutral-lightGray border border-neutral-lightGray'
                  }`}>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-neutral-darkGray uppercase tracking-wider ml-1">Name</label>
                  <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Emily" className="px-4 py-3.5 text-base placeholder-neutral-lightGray" autoFocus />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-neutral-darkGray uppercase tracking-wider ml-1">Grade Level</label>
          ...
          {step === 2 && (
            <div className="animate-in slide-in-from-right-8 fade-in duration-300">
              <div className="text-center mb-8">
                ...
                <p className="text-neutral-darkGray">Grades determine the hourly earning rate</p>
              </div>
              <div className="mb-8 p-6 rounded-none bg-neutral-mutedBg border border-neutral-lightGray text-center relative overflow-hidden group">
              ...
              <div className="space-y-4 mb-4">
                {subjects.map((sub, idx) => (
                  <div key={sub.id} className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                    <Input value={sub.name} onChange={(e) => handleUpdateSubject(sub.id, 'name', e.target.value)} placeholder="Subject Name" className="flex-[2] px-4 py-3 text-base placeholder-neutral-lightGray" />
                    <div className="flex items-center gap-3 w-full sm:w-auto flex-1">
                      ...
                      {subjects.length > 1 && (
                        <button onClick={() => handleRemoveSubject(sub.id)} className="w-11 h-11 flex items-center justify-center rounded-none border border-neutral-lightGray text-neutral-darkGray hover:bg-semantic-destructive/10 hover:text-semantic-destructive hover:border-semantic-destructive transition-colors cursor-pointer">
          ...
          {step === 3 && (
            <div className="animate-in zoom-in-95 fade-in duration-500 text-center">
              ...
              <div className="mb-8 p-6 rounded-none bg-neutral-mutedBg border border-neutral-lightGray">
                <p className="text-xs font-bold text-neutral-darkGray uppercase tracking-widest mb-2">Hourly Rate</p>
        ...
        <div className="relative z-10 px-8 py-6 border-t border-neutral-lightGray bg-neutral-mutedBg">
          <div className="flex gap-4">
            {step > 1 && (<button onClick={handleBack} className="flex-1 py-4 rounded-none font-bold text-neutral-darkGray bg-white border border-neutral-lightGray hover:bg-neutral-lightGray/10 transition-all cursor-pointer uppercase tracking-wider text-sm">Back</button>)}
            {step < 3 ? (
              <button onClick={handleNext} disabled={step === 1 && !isValidStep1} className={`flex-1 py-4 rounded-none font-bold text-white transition-all cursor-pointer uppercase tracking-wider text-sm ${(step === 1 && !isValidStep1) ? 'bg-neutral-lightGray text-neutral-darkGray cursor-not-allowed' : 'bg-primary-cardinal hover:bg-primary-cardinal/90 shadow-md'}`}>Next</button>
            ) : (
